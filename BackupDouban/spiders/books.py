...
class BooksSpider(scrapy.Spider):
    ...
    def parse_book(self, response):
        ...
        authors = book_info.xpath(
            "normalize-space(.//descendant::span[contains(text(), '作者')]/following-sibling::a/text())"
        ).getall()
        if not authors:
            authors = book_info.xpath(
                "normalize-space(.//span[contains(text(), '作者')]/following-sibling::a[count(preceding-sibling::span)=1]/text())"
            ).getall()
        translators = book_info.xpath(
            "normalize-space(.//descendant::span[contains(text(), '译者')]/following-sibling::a/text())"
        ).getall()
        if not translators:
            translators = book_info.xpath(
                "normalize-space(.//span[contains(text(), '译者')]/following-sibling::a[count(preceding-sibling::span)=1]/text())"
            ).getall()
        ...
        yield {
            ...
            "intro": response.xpath(
                'normalize-space(//span[@class="all hidden"]/descendant::div[@class="intro"]/p/text())'
            ).getall(),
        }
